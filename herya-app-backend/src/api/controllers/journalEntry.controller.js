...
/**
 * Controller: getDigitalGarden
 * -----------------------------
 * Retrieves data for the Digital Garden visualization.
 *
 * Returns:
 * - All journal entries formatted for garden display
 * - Each entry as a "flower" with size, color based on mood/energy
 * - Grouped by time periods (weeks/months)
 * - Emotional trend analysis
 * - Physical improvement tracking
 *
 * Workflow:
 * 1. Gets all user's journal entries.
 * 2. Transforms data for visualization.
 * 3. Calculates trends and insights.
 * 4. Returns formatted garden data.
 *
 * Error Handling:
 * - 401: User not authenticated
 *
 * Notes:
 * - Core feature for emotional/physical journey visualization.
 * - Data structured for d3.js or similar visualization library.
 * - Shows growth over time visually.
 */
const getDigitalGarden = async (req, res, next) => {
	try {
		// req.user verified by authenticateToken middleware
		const journals = await JournalEntry.find({ user: req.user._id })
			...
			.sort({ createdAt: 1 }); // Oldest first for chronological growth

		// Transform journals into "flowers" for visualization
		const flowers = journals.map((journal) => ({
			id: journal._id,
			...
			energyChange: journal.energyLevel.after - journal.energyLevel.before,
			stressChange: journal.stressLevel.before - journal.stressLevel.after,
			sequenceFamily: journal.vkReflection?.sequenceFamily,
			insights: journal.insights,
			gratitude: journal.gratitude,
		}));

		// Calculate trends
		const emotionalTrends = calculateEmotionalTrends(journals);
		const physicalProgress = calculatePhysicalProgress(journals);

		return sendResponse(res, 200, true, "Digital Garden data retrieved", {
			flowers,
			trends: {
			...
			firstEntry: journals[0]?.createdAt || null,
			latestEntry: journals[journals.length - 1]?.createdAt || null,
		});
	} catch (error) {
		return next(error);
	}
 ...
 */
function calculateEmotionalTrends(journals) {
 ...
 */
function calculatePhysicalProgress(journals) {
